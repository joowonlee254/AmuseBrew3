...
public class Playercontroller2_donghee : MonoBehaviour
{
    // 스피드 조정 변수
    [SerializeField]
    ...
    public bool isDead; // 캐릭터 죽음 확인 변수
    ...
    private void Move() // 캐릭터 방향키 움직임
    {
        ...
        Vector3 _moveHorizontal = transform.right * _moveDirX * Time.deltaTime; // transform이 갖고 있는 위치값에 오른쪽을 쓴다.
        Vector3 _moveVertical = transform.forward * _moveDirZ * Time.deltaTime; // 앞 뒤 빼고 위와 같음
